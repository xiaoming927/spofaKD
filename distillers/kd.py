...
class KD(BaseDistiller):
    ...
    def forward(self, image, label, *args, **kwargs):
        ...
        loss_kd = self.args.kd_loss_weight * kd_loss(
            logits_student, logits_teacher, self.args.kd_temperature
        )
...
class BKD(BaseDistiller):
    ...
    def forward(self, image, label, *args, **kwargs):
        ...
        loss_gt = self.args.gt_loss_weight * self.criterion(logits_student, label)
        loss_kd = self.args.kd_loss_weight * F.binary_cross_entropy_with_logits(
            logits_student, logits_teacher.softmax(1)
        )
        losses_dict = {"loss_gt": loss_gt, "loss_kd": loss_kd}
        return logits_student, losses_dict
