...
class OFA(BaseDistiller):
        ...
        for stage in self.args.ofa_stage:
            ...
            if is_cnn_student:
                ...
                if stage != 4:
                    ...
                    down_sample_blks = [
                        nn.Conv2d(in_chans, max(feature_dim_s, feature_dim_t), 1, 1, 0)
                    ]
                ...
                for i in range(merge_num):
                    if i == 0:  # proj to feature_dim_s
                        merger_modules.append(
                            PatchMerging(
                                input_resolution=(patch_grid // 2**i, patch_grid // 2**i),
                                dim=embed_dim,
                ...
                blocks = nn.Sequential(
                    *[
                        Block(dim=feature_dim_s, num_heads=4) for _ in range(max(4 - stage, 1))
                    ]  # todo: check this
                )
                if token_num != 0:
    ...
    def forward(self, image, label, *args, **kwargs):
        ...
        loss_kd = self.args.kd_loss_weight * ofa_loss(
            logits_student,
            logits_teacher,
            target_mask,
            self.args.ofa_eps[-1],
