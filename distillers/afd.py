...
@register_distiller
class AFD(BaseDistiller):
    requires_feat = True

    def __init__(self, student, teacher, criterion, args, **kwargs):
        super(AFD, self).__init__(student, teacher, criterion, args)
    ...
    def forward(self, image, label, *args, **kwargs):
        ...
        g_t = [feat_teacher[i] for i in self.args.afd_guide_layers]
        g_s = [feat_student[i] for i in self.args.afd_hint_layers]
        loss = self.attention(g_s, g_t)
        losses_dict = {"loss_gt": 0, "loss_kd": sum(loss)}
...
class Attention(nn.Module):
    ...
    def forward(self, g_s, g_t):
        ...
        logit = torch.add(torch.einsum("bstq,btq->bts", bilinear_key, query), p_logit) / np.sqrt(
            self.qk_dim
        )
...
class LinearTransformStudent(nn.Module):
    ...
    def forward(self, g_s):
        ...
        key = torch.stack(
            [key_layer(f_s) for key_layer, f_s in zip(self.key_layer, channel_mean)], dim=1
        ).view(
            bs * self.s, -1
        )  # Bs x h
...
class Sample(nn.Module):
    ...
    def forward(self, g_s, bs):
        g_s = torch.stack(
            [self.sample(f_s.pow(2).mean(1, keepdim=True)).view(bs, -1) for f_s in g_s], dim=1
        )
        return g_s
