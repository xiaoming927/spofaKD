...
# set default logging path to its own output folder, making multiple training at the same time available
# _logger = logging.getLogger("train")
_logger = None
_tb_logger = None  # tensorboard logger
_wd_logger = None  # wandb logger
...
parser.add_argument("--gt-loss-weight", default=1.0, type=float)
parser.add_argument("--kd-loss-weight", default=1.0, type=float)
...
# OFF parameters
parser.add_argument("--off-stage", default=[1, 2, 3, 4], nargs="+", type=int)
parser.add_argument("--off-embed-shape", default=[1, 196, 256], nargs="+", type=int)
parser.add_argument("--off-pl-loss-weight", default=1.0, type=float)
parser.add_argument("--off-pc-loss-weight", default=1.0, type=float)
parser.add_argument("--off-rc-loss-weight", default=1.0, type=float)
parser.add_argument("--off-afd", action="store_true", default=False)
parser.add_argument("--off-afd-dim", default=512, type=int)
parser.add_argument("--off-warmup-epochs", default=20, type=int)

# pat parameters
parser.add_argument("--pat-afp-stage", default=[1, 2, 3, 4], nargs="+", type=int)
parser.add_argument("--pat-mse-loss-weight", default=1.0, type=float)
parser.add_argument("--pat-reg-loss-weight", default=1.0, type=float)
parser.add_argument("--pat-raa-num-queries", default=64, type=int)
parser.add_argument("--pat-raa-dim", default=512, type=int)
...
# FitNet parameters
parser.add_argument("--fitnet-stages", default=[1, 2, 3, 4], nargs="+", type=int)
...
parser.add_argument("--weight-decay", type=float, default=2e-5, help="weight decay (default: 2e-5)")
parser.add_argument(
    "--clip-grad",
    type=float,
    default=None,
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="whether to recovery training from saved checkpoint",
)
parser.add_argument(
    "--recovery-interval",
    type=int,
...
def main():
    ...
    if args.teacher:
        ...
        strict = True
        if (
            args.dataset == "imagenet" and args.teacher == "convnext_tiny"
        ):  # FIXME: Special case for convnext on imagenet
            strict = False
        load_checkpoint(teacher, args.teacher_pretrained, use_ema=args.use_ema_teacher, strict=strict)
        if Distiller.requires_feat:
    ...
    # FIXME: Please, write the resume code in a better format
    # setup tmp checkpoint saver and eval metric tracking for checkpoint reloading
    if args.recovery:
        output_dir = None
        if args.experiment:
        ...
        tmp_saver = CheckpointSaver(
            model=None,
            optimizer=None,
            args=args,
            model_ema=None,  # save distiller's state_dict in model_ema slot
            amp_scaler=None,
        ...
        checkpoint_path = tmp_saver.find_recovery()
        checkpoint = torch.load(checkpoint_path, map_location=torch.device("cuda"))
        # FIXME: Find a better way to recover the distiller's state
        if hasattr(distiller, "module"):
            distiller.module.load_state_dict(checkpoint["state_dict_ema"])
        else:
            distiller.load_state_dict(checkpoint["state_dict_ema"])
        ...
        if "metric" in checkpoint.keys():
            best_metric, best_epoch = checkpoint["metric"], start_epoch
        ...
        if model_emas is not None:
            for ema, decay in model_emas:
                ...
                checkpoint_path = tmp_ema_saver.find_recovery()
                checkpoint = torch.load(checkpoint_path, map_location=torch.device("cuda"))
                if hasattr(ema, "module"):
                    ema.module.load_state_dict(checkpoint["state_dict_ema"])
                else:
                    ema.load_state_dict(checkpoint["state_dict_ema"])
                if "metric" in checkpoint.keys():
                    best_metric_ema, best_epoch_ema = checkpoint["metric"], checkpoint["epoch"]
    ...
    if args.rank == 0:
        ...
        saver = CheckpointSaver(
            ...
            optimizer=optimizer,
            args=args,
            model_ema=distiller,  # save distiller's state_dict in model_ema slot
        ...
        # setup tensorboard & wandb logger
        global _tb_logger
        _tb_logger = SummaryWriter(os.path.join(output_dir, "train_events"))
        global _logger
        _logger = logging.getLogger("train")
        # logging.basicConfig(
        #     level=logging.DEBUG,
        ...
        #     filemode="w",
        # )
        setup_default_logging(log_path=os.path.join(output_dir, "train.log"))

        # Wandb logger
        if has_wandb:
            # assert has_wandb, "Run 'pip install wandb' first to enable Wandb logging."
            # has_wandb = True
            tags = [safe_model_name(args.model), safe_model_name(args.teacher)]
            wandb.init(
                project=f"OF2KD_{args.dataset}",
                name=exp_name,
                tags=tags,
            ...
            wandb.run.log_code(
                "./distillers",
                include_fn=lambda path: path.endswith(f"{args.distiller}.py") or path.endswith(".ipynb"),
        ...
        if model_emas is not None:
            ...
            for ema, decay in model_emas:
                ...
                if args.recovery:
                    checkpoint_path = ema_saver.find_recovery()
                    checkpoint = torch.load(checkpoint_path, map_location=torch.device("cuda"))
                    if hasattr(ema, "module"):
                        ema.module.load_state_dict(checkpoint["state_dict_ema"])
                    else:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if not args.speedtest and is_eval:
                eval_metrics = validate(
                    model, loader_eval, validate_loss_fn, args, epoch=epoch, amp_autocast=amp_autocast
                )
                ...
                if model_emas is not None and not args.model_ema_force_cpu:
                    for j, ((ema, decay), ema_saver) in enumerate(zip(model_emas, ema_savers)):
                        ...
                        if ema_saver is not None:
                            ...
                            best_metric_ema, best_epoch_ema = ema_saver.save_checkpoint(epoch, metric=save_metric)
                            ema_saver.save_recovery(epoch)
                            if best_metric_ema > best_metric:
                                best_metric = best_metric_ema
                                best_epoch = best_epoch_ema

                if args.rank == 0:
                    _tb_logger.add_scalar(f"Best_Metrics/Acc@1", best_metric, epoch)
                    if has_wandb:
                        wandb.log({f"Best_Metrics/Acc@1": best_metric})
    ...
    if best_metric is not None:
        if args.rank == 0:
            ...
            # Uncomment this to upload checkpoints to wandb
            # checkpoint_path = os.path.join(output_dir, "checkpoint", "model_best.pth.tar")
            # wandb.log_artifact(checkpoint_path, name="model_best", type="model")
            # checkpoint_path = os.path.join(output_dir, "ema0.9998_checkpoint", "model_best.pth.tar")
            # wandb.log_artifact(checkpoint_path, name="ema_model_best", type="model")
            # wandb.finish()
...
def train_one_epoch(
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.rank == 0:
                ...
                for k, v in losses_m_dict.items():
                    ...
                    _tb_logger.add_scalar(f"Loss/{k.capitalize()}", v.val, epoch)
                    if has_wandb:
                        wandb.log({f"Loss/{k.capitalize()}": v.val})
                        if np.isnan(v.val) or np.isnan(v.avg):
                            wandb.alert(title="Loss is NaN", text=f"Loss/{k.capitalize()} is nan")
                            raise ValueError(f"Loss/{k.capitalize()} is nan")
                ...
                _tb_logger.add_scalar("LR", lr, epoch)
                _tb_logger.flush()
                if has_wandb:
                    wandb.log({f"LR": lr})
    ...
    if args.rank == 0:
        _tb_logger.add_scalar("Loss/Loss", losses_m.val, epoch)
        _tb_logger.flush()
        if has_wandb:
            wandb.log({f"Loss/Loss": losses_m.avg})
...
def validate(model, loader, loss_fn, args, epoch, amp_autocast=suppress, log_suffix=""):
    ...
    if args.rank == 0:
        n = "Metrics_EMA" if "EMA" in log_suffix else "Metrics"
        _tb_logger.add_scalar(f"{n}/Acc@1", top1_m.avg, epoch)
        _tb_logger.add_scalar(f"{n}/Acc@5", top5_m.avg, epoch)
        _tb_logger.flush()

        if has_wandb:
            wandb.log({f"{n}/Acc@1": top1_m.avg})
            wandb.log({f"{n}/Acc@5": top5_m.avg})
