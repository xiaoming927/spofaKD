...
class HireMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        pad_h, pad_w = (
            (self.pixel - H % self.pixel) % self.pixel,
            (self.pixel - W % self.pixel) % self.pixel,
        )
        h, w = x.clone(), x.clone()
        ...
        h = (
            h.reshape(B, C, (H + pad_h) // self.pixel, self.pixel, W)
            .permute(0, 1, 3, 2, 4)
            .reshape(B, C * self.pixel, (H + pad_h) // self.pixel, W)
        )
        w = (
            w.reshape(B, C, H, (W + pad_w) // self.pixel, self.pixel)
            .permute(0, 1, 4, 2, 3)
            .reshape(B, C * self.pixel, H, (W + pad_w) // self.pixel)
        )
        ...
        h = (
            h.reshape(B, C, self.pixel, (H + pad_h) // self.pixel, W)
            .permute(0, 1, 3, 2, 4)
            .reshape(B, C, H + pad_h, W)
        )
        w = (
            w.reshape(B, C, self.pixel, H, (W + pad_w) // self.pixel)
            .permute(0, 1, 3, 4, 2)
            .reshape(B, C, H, W + pad_w)
        )
    ...
    def __init__(
        ...
        for i in range(len(layers)):
            stage = basic_blocks(
                ...
                pixel=pixel[i],
                step_stride=step_stride[i],
                step_dilation=step_dilation[i],
                step_pad_mode=step_pad_mode,
                pixel_pad_mode=pixel_pad_mode,
            )
